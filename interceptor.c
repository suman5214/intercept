...
asmlinkage long interceptor(struct pt_regs reg) {
    ...
    if(table[sysc].intercepted) {
	    if (table[sysc].monitored == 2 && !(check_pid_monitored(sysc,current->pid))){
	    	...
	    	unsigned long b = (unsigned long)reg.bx;
	        log_message(current->pid, sysc, b, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
	    }
	 	...
	 	if(cmd == REQUEST_SYSCALL_INTERCEPT) {
			...
			if(table[syscall].monitored == 2 || check_pid_monitored(syscall,pid) == 1){
	 		...
	 		if (pid == 0){
		   	...
		   	printk(KERN_DEBUG "start table intercepted:%d monitored:%d  length:%d PID:%d\n",table[syscall].intercepted,table[syscall].monitored,table[syscall].listcount,pid);
